...
//! This pool manages active TLS connections to target servers via the proxy.
...
/// Represents a pooled TLS connection along with its usage metadata.
type TlsConnection = SslStream<TcpStream>;
        ...
        // 1. Check Pool (Acquire write lock only for this specific shard)
        {
            ...
            if let Some(pooled) = shard.remove(&target_key) {
                ...
                if pooled.last_used.elapsed() < self.config.pool_idle_timeout {
                    ...
                    debug!("Pool connection for {target_key} expired, dropping.");
                }
            }
        }
