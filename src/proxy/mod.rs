...
pub use protocol::{ProxyProtocol, ProxyProtocolType, ProtocolRegistry};

/// HTTP CONNECT tunnel proxy connector.
///
/// Implements the `ProxyProtocol` trait for HTTP CONNECT tunneling.
...
impl ProxyConnector {
    ...
    pub async fn connect(&self, target: &Uri) -> ProxyResult<TcpStream> {
        self.establish_tunnel(target).await
    }
    ...
    async fn execute_connect(
        ...
        let host = target
            .host()
            .ok_or_else(|| ProxyError::InvalidUrl("Missing target host".into()))?;
...
#[async_trait]
impl ProxyProtocol for ProxyConnector {
    fn protocol_type(&self) -> ProxyProtocolType {
        ProxyProtocolType::HttpConnect
    }
    ...
    async fn establish_tunnel(&self, target: &Uri) -> ProxyResult<TcpStream> {
        ...
        let target_host = target.host().unwrap_or("unknown");
        let target_port = target.port().map(|p| p.as_u16()).unwrap_or(443);

        info!(
            "Connecting to proxy {proxy_host}:{proxy_port} -> {target_host}:{target_port}"
        );
